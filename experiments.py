...
from resnet_FedMLB import *


def KD(input_p, input_q, T=1):
    kl_loss = nn.KLDivLoss(reduction="batchmean")  # KL散度损失函数
    p = F.softmax(input_p/T, dim=1)  # input_p的softmax函数
    q = F.log_softmax(input_q/T, dim=1)  # input_q的log_softmax函数
    result = kl_loss(q, p)  # 计算KL散度
    return result    

'''
def KD(input_p, input_q, temp):
    T = temp.cuda()
    KD_loss = 0
    KD_loss += nn.KLDivLoss(reduction='batchmean')(F.log_softmax(input_p/T, dim=1),
                                F.softmax(input_q/T, dim=1)) * T * T
    return KD_loss
...
def train_net_orifedmlb(net_id, net, global_model, train_dataloader, test_dataloader, epochs, lr, args_optimizer, device="cpu"):
    # 输出客户端训练信息
    logger.info('Training network %s' % str(net_id))
    # 计算训练和测试准确率，包括混淆矩阵计算
    train_acc = compute_accuracy(net, train_dataloader, device=device)
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, trainable_list.parameters()), lr=lr, momentum=args.rho, weight_decay=args.reg)
    ...
    for epoch in range(epochs):
        ...
        for tmp in train_dataloader:
            ...
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                target = target.long()
                # 计算前向传播
                out_of_local = net(x, return_feature=True)
                ...
                for it in range(num_branch):    
                    this_log_prob = global_model(local_features[it], level=it + 1)  # 全局模型
                    this_ce = criterion(this_log_prob, target)  # 交叉熵损失
                    
                    this_kl = KD(this_log_prob, log_probs, 1)  # KL散度损失
                    log_prob_branch.append(this_log_prob)  # 分支输出的log_softmax损失
                    ce_branch.append(this_ce)  # 分支输出的交叉熵损失
                    kl_branch.append(this_kl)  # 分支输出的KL散度损失

                # 计算损失
                ce_loss = criterion(log_probs, target)
                loss = 1 * ce_loss + 1 * (sum(ce_branch)) / num_branch + 1 * (sum(kl_branch)) / num_branch  # 总的损失函数
                loss.backward()
                # 更新权重
                optimizer.step()
                # 记录该epoch的loss
...
def train_net_fedmlb(temp_list, mlp, net_id, net, global_model, train_dataloader, test_dataloader, epochs, lr, args_optimizer, device="cpu"):
    # 输出客户端训练信息
    logger.info('Training network %s' % str(net_id))
    # 计算训练和测试准确率，包括混淆矩阵计算
    train_acc = compute_accuracy(net, train_dataloader, device=device)
    ...
    #writer = SummaryWriter()
    tau = 0
    temp = temp_list[net_id]

    # 进行epoch的循环
    for epoch in range(epochs):
        epoch_loss_collector = []
        decay_value = gradient_decay.get_value(epoch)
        #print('decay_value', decay_value)
        mlp.train()
        ...
        for tmp in train_dataloader:
            ...
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                for it in range(num_branch):    
                    this_log_prob = global_model(local_features[it], level=it + 1)  # 全局模型
                    this_ce = criterion(this_log_prob, target)  # 交叉熵损失
                    temp = mlp(this_log_prob, log_probs, decay_value)  # (teacher_output, student_output)
                    temp = 0.5 + 1 * torch.sigmoid(temp)
                    temp = temp.cuda()
                    this_kl = KD(this_log_prob, log_probs, temp)  # KL散度损失
                    log_prob_branch.append(this_log_prob)  # 分支输出的log_softmax损失
                    ce_branch.append(this_ce)  # 分支输出的交叉熵损失
                    kl_branch.append(this_kl)  # 分支输出的KL散度损失
    ...
    logger.info(' ** Training complete **')
    # 返回训练和测试准确率，特定梯度和其他信息
    return train_acc, test_acc, a_i, norm_grad, temp
...
def local_train_net(nets, selected, args, net_dataidx_map, test_dl = None, device="cpu"):
    ...
    wandb.log({"avg_test_acc": avg_acc})
...
def local_train_net_fednova(nets, selected, global_model, args, net_dataidx_map, test_dl = None, device="cpu"):
    ...
    for net_id, net in nets.items():
        ...
        train_dl_global, test_dl_global, _, _ = get_dataloader(args.dataset, args.datadir, args.batch_size, 32)
        n_epoch = args.epochs
        # 训练客户端模型并返回结果, 包括训练准确度、测试准确度、特定的梯度信息、每个epoch的loss等
    ...
    if args.alg == 'local_training':
        logger.info("avg test acc %f" % avg_acc)
    # 将训练好的模型列表进行聚合，返回聚合后的模型列表以及其他信息
    nets_list = list(nets.values())
    wandb.log({"avg_test_acc": avg_acc})
...
def local_train_net_orifedmlb(nets, selected, global_model, args, net_dataidx_map, test_dl = None, device="cpu"):
    # 初始化平均准确率
    avg_acc = 0.0
    # 把全局模型转到特定设备
    global_model.to(device)
    ...
    for net_id, net in nets.items():
        ...
        # 训练客户端模型并返回结果, 包括训练准确度、测试准确度、特定的梯度信息、每个epoch的loss等
        trainacc, testacc = train_net_orifedmlb(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, device=device)
...
def local_train_net_fedmlb(temp_list, nets, selected, global_model, args, net_dataidx_map, test_dl = None, device="cpu"):
    # 初始化平均准确率
    avg_acc = 0.0
    # 初始化梯度信息列表和数据量列表
    a_list = []
    ...
    for net_id, net in nets.items():
        ...
        # 训练客户端模型并返回结果, 包括训练准确度、测试准确度、特定的梯度信息、每个epoch的loss等
        trainacc, testacc, a_i, d_i, temp = train_net_fedmlb(temp_list , mlp, net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, device=device)

        a_list.append(a_i)
        d_list.append(d_i)
        # 用temp_list存下每个客户端对应的temp
        temp_list[net_id] = temp
        n_i = len(train_dl_local.dataset)
        n_list.append(n_i)
        logger.info("net %d final test acc %f" % (net_id, testacc))
        avg_acc += testacc
    ...
    # 将训练好的模型列表进行聚合，返回聚合后的模型列表以及其他信息
    nets_list = list(nets.values())
    return nets_list, a_list, d_list, n_list, temp_list
...
if __name__ == '__main__':
    ...
    args = get_args()
    mkdirs(args.logdir)
    wandb.init(project="NIID-Bench", name=args.alg, config=args)
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.comm_round):
            ...
            _ = local_train_net_orifedmlb(nets, selected, global_model, args, net_dataidx_map, test_dl = test_dl_global, device=device)
            # local_train_net(nets, args, net_dataidx_map, local_split=False, device=device)
        ...
        # 初始化temp_list，大小是net的个数，初始值为1
        temp_list = [1 for i in range(args.n_parties)]
        ...
        for round in range(args.comm_round):
            ...
            _, a_list, d_list, n_list, temp_list = local_train_net_fedmlb(temp_list, nets, selected, global_model, args, net_dataidx_map, test_dl = test_dl_global, device=device)
            total_n = sum(n_list)
            #print("total_n:", total_n)
            d_total_round = copy.deepcopy(global_model.state_dict())
            for key in d_total_round:
